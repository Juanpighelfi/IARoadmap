...
| --- | --- | --- | --- | --- |
| JavaScript | variables | en aprendizaje | Distingue conceptualmente `let` de `const` y referencias, pero en el ejercicio integrador declaró el acumulador `inventario` con `const` y luego intentó reasignarlo; falta transferir la regla al código propio. | 2026-09-10 |
...
| JavaScript | funciones | en aprendizaje | Comprende conceptualmente que `return` devuelve un valor y termina la función, pero en el ejercicio integrador omitió devolver el acumulado; falta transferir `return` al código propio. | 2026-09-10 |
